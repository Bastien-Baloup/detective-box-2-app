...
const EventHandler = () => {
	const { actionToggleDataEvent, actionToggleDataHistory, toggleDataEvent } = useContext(DataContext)
	...
	const openModaleMalle = useCallback(() => setModaleMalle(true), [setModaleMalle])
	...
	const openModaleSquelette = useCallback(() => setModaleSquelette(true), [setModaleSquelette])
	const openEndGameModale = useCallback(() => setEndGameModale(true), [setEndGameModale])
	...
	useEffect(() => {
		...
		if (currentBox === 1 && state.id === 'box1document6' && !modaleInterrogatoireGarraud) {
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<p>
						Attention, j&apos;ai écouté l&apos;enregistrement et il est assez dur. Pour les âmes sensibles, cliquez sur
						la transcription.
					</p>
					...
					<button type='button' className='modal-objectif__button button--red' onClick={handleOpenRebeccaAudio}>
			...
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<p>
						...
						fin... La VHS est entre les mains de Tim pour de plus amples analyses. N&apos;hésitez pas à le solliciter
						pour accéder au contenu.
					</p>
					<p>PLAN</p>
					<p>
						Il y avait aussi ce plan étrange, il n&apos;a pas l&apos;air bien vieux, il a dû être accroché à la maison
						il n&apos;y a pas longtemps.
					</p>
					<p>Ça ne correspond à aucune des adresses qu&apos;on a trouvées jusqu&apos;ici...</p>
					<p>Vous pouvez l&apos;étudier depuis l&apos;Historique</p>
					<button type='button' className='modal-objectif__button button--red' onClick={handleCloseModaleVHS}>
						Continuer l&apos;enquête
					</button>
				</div>
			</div>
	...
	const displayModaleInterrogatoireGarraud = () => {
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					<p>
						Ça fait de sacrées révélations tout ça... Je pense qu&apos;avec ce qu&apos;on a là, on devrait pouvoir
						interroger Charles Garraud.
					</p>
					<button type='button' className='modal-objectif__button button--red' onClick={handleOpenInterrogatoire}>
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<p>
						Un squelette entier, je ne sais pas de quand il date, mais c&apos;est sacrément bien conservé, sauf la tête.
					</p>
					...
					<button type='button' className='modal-objectif__button button--red' onClick={handleCloseModaleSquelette}>
						Continuer l&apos;enquête
					</button>
				</div>
			</div>
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box modal-objectif__endGame'>
					{renderEndText()}
					{currentBox === 3 ? (
						<button type='button' className='modal-objectif__button button--red' onClick={handleEndGameModale}>
							Classer l&apos;affaire
						</button>
					) : (
						<button type='button' className='modal-objectif__button button--red' onClick={handleEndGameModale}>
