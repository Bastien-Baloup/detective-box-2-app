...
const Objectif = ({ data }) => {
	...
	const { updateHelp, updateEvent, updateHistory, updateObjectives } = useApi()
	...
	const event35 = useMemo(
		() =>
			currentBox === 3 &&
			dataEvent[currentBox]?.data &&
			dataEvent[currentBox]?.data.find((event) => event.id === 35)?.status,
		[currentBox, dataEvent]
	)

	const box1lieu2 = useMemo(
		() =>
			currentBox === 1 &&
			dataHistory[currentBox]?.data &&
			dataHistory[currentBox]?.data.find((event) => event.id === 'box1lieu2')?.status,
		[currentBox, dataHistory]
	)
	const box2lieu1 = useMemo(
		() =>
			currentBox === 2 &&
			dataHistory[currentBox]?.data &&
			dataHistory[currentBox]?.data.find((event) => event.id === 'box2lieu1')?.status,
		[currentBox, dataHistory]
	)
	const box2lieu3 = useMemo(
		() =>
			currentBox === 2 &&
			dataHistory[currentBox]?.data &&
			dataHistory[currentBox]?.data.find((event) => event.id === 'box2lieu3')?.status,
		[currentBox, dataHistory]
	)
	...
	const box3lieu2 = useMemo(
		() =>
			currentBox === 3 &&
			dataHistory[currentBox]?.data &&
			dataHistory[currentBox]?.data.find((event) => event.id === 'box3lieu2')?.status,
		[currentBox, dataHistory]
	)
	const box3lieu3 = useMemo(
		() =>
			currentBox === 3 &&
			dataHistory[currentBox]?.data &&
			dataHistory[currentBox]?.data.find((event) => event.id === 'box3lieu3')?.status,
		[currentBox, dataHistory]
	)

	const box3help4 = useMemo(
		() =>
			currentBox === 3 &&
			dataHelp[currentBox]?.data &&
			dataHelp[currentBox]?.data.find((event) => event.id === 'box3help4')?.status,
		[currentBox, dataHelp]
	)

	const objectif11 = useMemo(
		() =>
			currentBox === 1 &&
			dataObjectif[currentBox]?.data &&
			dataObjectif[currentBox]?.data.find((event) => event.id === 11)?.status,
		[currentBox, dataObjectif]
	)
	const objectif12 = useMemo(
		() =>
			currentBox === 1 &&
			dataObjectif[currentBox]?.data &&
			dataObjectif[currentBox]?.data.find((event) => event.id === 12)?.status,
		[currentBox, dataObjectif]
	)
	const objectif13 = useMemo(
		() =>
			currentBox === 1 &&
			dataObjectif[currentBox]?.data &&
			dataObjectif[currentBox]?.data.find((event) => event.id === 13)?.status,
		[currentBox, dataObjectif]
	)
	const objectif34 = useMemo(
		() =>
			currentBox === 3 &&
			dataObjectif[currentBox]?.data &&
			dataObjectif[currentBox]?.data.find((event) => event.id === 34)?.status,
		[currentBox, dataObjectif]
	)
			...
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					{displayTextMauvaiseFin1 ? (
						<>
							<p>
								Ce n&apos;était pas la bonne cible, nous nous sommes trompés... Céline est malheureusement dans la
								nature et elle a réussi son grand œuvre.
							</p>
							<p>Souhaitez-vous réessayer le dernier objectif ou passer à l&apos;épilogue ?</p>
							<button type='button' className='modal-objectif__button button--red' onClick={handleReset}>
								Recommencer
							</button>
							<button type='button' className='modal-objectif__button button--red' onClick={handleGoToResolution}>
				...
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
						<button type='button' className='modal-objectif__icon--container'>
							<img className='modal-objectif__icon' src={Cross} onClick={handleModal} alt='' />
						</button>
						...
						<button type='button' className='modal-objectif__button button--red' onClick={handleModal}>
							Continuer l&apos;enquête
						</button>
					</div>
				</div>
			...
			return (
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
						...
						<p className='modal-objectif__subtitle'>
							On a le nom des deux dernières cibles, mais il n&apos;en reste plus qu&apos;une en vie, il faut qu&apos;on
							la trouve pour pouvoir la sauver.
						</p>
			...
			return (
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
						...
						<p className='modal-objectif__subtitle'>
							Il faut qu&apos;on sache où est cette personne pour intervenir à temps, on ne peut pas se permettre de se
							tromper
						</p>
			...
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					{displayButtonCelineTel ? (
						<button type='button' className='modal-objectif__button button--red' onClick={handleFinalStep}>
							Continuer l&apos;enquête
						</button>
					) : (
						''
			...
			return (
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
									...
									return (
										<div key={i} className='modal-objectif__victimes__info'>
											<div className='modal-objectif__victimes__photos--container'>
												<img className='modal-objectif__victimes__photos' src={urlApi.cdn() + el.img} alt='' />
											</div>
						...
						<div className='modal-objectif__errorMessage'>{errorMessage}</div>
						<button type='button' className='modal-objectif__button button--red' onClick={handleSubmit14}>
							Valider
						</button>
					</div>
				</div>
			...
			return (
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
						...
						<div className='modal-objectif__errorMessage'>{errorMessage}</div>
						<button type='button' className='modal-objectif__button button--red' onClick={handleSubmit21}>
							Valider
						</button>
					</div>
				</div>
			...
			return (
				<div className='modal-objectif__background'>
					<div className='modal-objectif__box'>
						...
						<div className='modal-objectif__errorMessage'>{errorMessage}</div>
						<button type='button' className='modal-objectif__button button--red' onClick={handleSubmit23}>
							Valider
						</button>
					</div>
				</div>
	...
	const renderModalBis = () => {
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					<button type='button' className='modal-objectif__icon--container'>
						<img className='modal-objectif__icon' src={Cross} onClick={handleModalBis} alt='' />
					</button>
					<h2 className='modal-objectif__title'>
						Objectif : <br /> {data.title}
					</h2>
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<button type='button' className='modal-objectif__button button--red' onClick={handleModalAnswerBis}>
						Continuer l&apos;enquête
					</button>
				</div>
			</div>
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<button type='button' className='modal-objectif__button button--red' onClick={handleDoneObjectifModal}>
						Valider
					</button>
				</div>
			</div>
	...
	const renderObjectif = () => {
		if (data.status === 'done' || currentBoxStatus === 'done') {
			if (currentBoxStatus === 'open' && (data.id === 14 || data.id === 24)) {
				return (
					<>
						<button type='button' className='objectif objectif--open' onClick={handleDoneObjectifModal}>
			...
			return (
				<>
					<button type='button' className='objectif objectif--done' onClick={handleDoneObjectifModal}>
						<div className='objectif__mainInfo'>
							<div className='objectif__icon-wrapper'>
		...
		if (data.status === 'open') {
			return (
				<>
					<button type='button' className='objectif objectif--open' onClick={nextStep ? handleModalBis : handleModal}>
						<div className='objectif__mainInfo'>
							<div className='objectif__icon-wrapper'>
								<img src={LockOpen} className='objectif__icon' alt='objectif ouvert' />
							</div>
			...
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box modal-hacking'>
					...
					{displayButtonOpenMail ? (
						<button type='button' className='modal-objectif__button button--red' onClick={handleOpenMailHacking}>
							Valider
						</button>
					) : (
						''
		...
		return (
			<div className='modal-objectif__background'>
				<div className='modal-objectif__box'>
					...
					<button type='button' className='modal-objectif__button button--red' onClick={handleCloseAudioBreakingNews}>
						Regarder les infos
					</button>
				</div>
			</div>
				...
				<div className='modal-objectif__box'>
					...
					{displayTextResolution ? (
						<>
							...
							<button type='button' className='modal-objectif__button button--red' onClick={handleInterrogatoireFinal}>
