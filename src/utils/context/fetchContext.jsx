...
export const BoxProvider = ({ children }) => {
	...
	return (
		<BoxContext.Provider value={{ setCurrentBox, currentBox, currentBoxStatus, setCurrentBoxStatus }}>
			{children}
		</BoxContext.Provider>
	)
}
...
export const DataProvider = ({ children }) => {
	...
	const fetchInitialData = async () => {
		...
		const resultEvent = [await getEventByBox(token, 1), await getEventByBox(token, 2), await getEventByBox(token, 3)]
		setDataEvent(resultEvent)

		// Fetch data for Help
		const resultHelp = [await getHelpByBox(token, 1), await getHelpByBox(token, 2), await getHelpByBox(token, 3)]
