...
const Lauren = ({ closeAgentPage }) => {
	...
	const { actionToggleDataLauren, dataLauren, actionToggleDataHistory, dataHistory } = useContext(DataContext)
	const {
	...
	const box2document6 = useMemo(
		() =>
			currentBox === 2 && dataHistory?.data && dataHistory?.data.find((event) => event.id === 'box2document6')?.status,
	...
	const thisBox = useMemo(
		() => dataLauren?.find((element) => element.box_id === currentBox)?.data,
		[currentBox, dataLauren]
	)
